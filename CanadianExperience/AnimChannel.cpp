 ...
 * indices.
 * 
 * \param currFrame The current frame that the Keyframe should be at
 */
 ...
 * Looks for a keyframe at the current frame
 */
void CAnimChannel::DeleteKeyframe()
{
	...
	{
		if (keyframe->GetFrame() == currFrame)
		{
			auto loc = find(begin(mKeyframes), end(mKeyframes), keyframe);

			// If the keyframe's frame equals the current frame, delete it, update keyframe1 & 2
